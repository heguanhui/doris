...
class FtpFileSystem final : public RemoteFileSystem {
public:
    static Result<std::shared_ptr<FtpFileSystem>> create(
            const std::map<std::string, std::string>& properties, std::string id);
    ~FtpFileSystem() override = default;
...
private:
    FtpFileSystem(Path&& root_path, std::string id,
